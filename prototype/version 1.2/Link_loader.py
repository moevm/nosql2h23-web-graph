            ...
            if i:
                ...
                if temp_domain:
                    ...
                    if not db.run_query('MATCH (n:' + str(temp_domain) + ' {url:"' + str(i) + '"}) RETURN n'):
                        ...
                        db.run_query('MATCH (n:' + str(domain) + ' {url: "' + str(link) + '"}) MATCH (s:' + str(
                            temp_domain) + '{url: "' + str(
                            i) + '"}) CREATE (n)-[m:LEADS_TO]->(s) SET m.datatime = "' + str(
                            datetime.datetime.now()) + '"')

                    elif not db.run_query(
                            'MATCH (n:' + str(domain) + ' {url:"' + str(link) + '"})-[m:LEADS_TO]->(s:' + str(
                                temp_domain) + ' {url:"' + str(i) + '"}) RETURN m') and link != i:
                        db.run_query('MATCH (n:' + str(domain) + ' {url: "' + str(link) + '"}) MATCH (s:' + str(
                            temp_domain) + '{url: "' + str(
                            i) + '"}) CREATE (n)-[m:LEADS_TO]->(s) SET m.datatime = "' + str(
                            datetime.datetime.now()) + '"')
